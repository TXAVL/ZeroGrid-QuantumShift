...
import '../config/txa_config.dart';
import '../localization/txa_language.dart';

class TXADeviceInfo {
  /// Kiểm tra thiết bị có bị Root (Android) hoặc Jailbreak (iOS) không
  ...
  /// Thu thập thông số phần cứng, pin, hệ điều hành & chẩn đoán chi tiết
  static Future<Map<String, String>> getDiagnosticMetadata() async {
    ...
    try {
      ...
      // 2. Độ phân giải màn hình & Pixel ratio
      try {
        ...
        if (views.isNotEmpty) {
          ...
          specs[txaLang.getText('diag_screen')] =
              '${size.width.toInt()}x${size.height.toInt()} px (${dpWidth}x$dpHeight dp @${ratio.toStringAsFixed(1)}x)';
        }
      } catch (_) {}

      // 3. Múi giờ, Locale & CPU
      ...
      // 4. Mức pin & Trạng thái sạc
      try {
        final battery = Battery();
        final batteryLevel = await battery.batteryLevel;
        final batteryState = await battery.batteryState;
        final isCharging = batteryState == BatteryState.charging || batteryState == BatteryState.full;
        final chargingText = isCharging
            ? txaLang.getText('diag_battery_charging')
            : txaLang.getText('diag_battery_discharging');
      ...
      // 5. Thông tin hệ điều hành & thiết bị
      if (Platform.isAndroid) {
        final androidInfo = await deviceInfoPlugin.androidInfo;
        specs[txaLang.getText('diag_platform_os')] =
            'Android ${androidInfo.version.release} (SDK ${androidInfo.version.sdkInt})';
        specs[txaLang.getText('diag_device')] =
            '${androidInfo.manufacturer.toUpperCase()} ${androidInfo.model} (${androidInfo.device})';
        ...
        specs[txaLang.getText('diag_ram')] =
      ...
      // 6. Thông tin phiên bản App
      specs[txaLang.getText('diag_app_name')] = TxaConfig.appName;
      specs[txaLang.getText('diag_app_version')] = TxaConfig.version;
      specs[txaLang.getText('diag_build_code')] = TxaConfig.buildNumber;
      specs[txaLang.getText('diag_full_version')] = TxaConfig.fullVersion;
      specs[txaLang.getText('diag_release_date')] = TxaConfig.releaseDate;
    } catch (e) {
  ...
  /// Tạo Header chẩn đoán định dạng chuẩn khi người dùng bấm copy log
