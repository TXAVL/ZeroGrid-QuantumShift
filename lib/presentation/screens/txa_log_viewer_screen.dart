...
class _TXALogViewerScreenState extends State<TXALogViewerScreen>
  ...
  Color _getTypeColor(String type) {
    ...
    } else if (upper.contains('IAP') || upper.contains('IAU') || upper.contains('GPGS')) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            tooltip: txaLang.getText('log_refresh_tooltip'),
            icon: const Icon(Icons.refresh_rounded, color: Colors.white70, size: 22),
            onPressed: _loadLogs,
          ),
          IconButton(
            tooltip: txaLang.getText('log_share_tooltip'),
            icon: const Icon(Icons.share_rounded, color: Color(0xFFFFD700), size: 20),
            onPressed: () async {
              TxaToast.info(context, txaLang.getText('log_sharing_prep'));
              await TXALogger.shareLogs(activeType);
            },
          ),
          IconButton(
            tooltip: txaLang.getText('log_clear_tooltip'),
            icon: const Icon(Icons.delete_sweep_rounded, color: Colors.redAccent, size: 22),
            onPressed: () async {
              await TXALogger.clearLogs();
              await _loadLogs();
              if (context.mounted) {
                TxaToast.success(context, txaLang.getText('log_cleared'));
              }
            },
          ),
            ...
            : _parsedEntries.isEmpty
                ? Center(
                    child: Column(
                      ...
                      children: [
                        Icon(Icons.assignment_outlined, size: 54, color: Colors.white.withValues(alpha: 0.3)),
...
''';
    ...
    try {
      ...
      if (mounted) {
        TxaToast.success(context, txaLang.getText('log_copied'));
      }
    } catch (e) {
      if (mounted) {
        TxaToast.error(
